...
# ---------------------------------------------------------------------------
# 1. yahoo_client/detail.py — try block内変数的未定義参照回避
# ---------------------------------------------------------------------------


def test_yahoo_detail_init_outside_try_blocks():
    """KIK-743: try 外 dividend_paid_history 等初期化已被source確認.

    実 API mock複雑的、source静的検査初期化位置担保执行。
    """
    from pathlib import Path
    src = Path(__file__).resolve().parent.parent / "src/data/yahoo_client/detail.py"
    text = src.read_text()
    # cashflow block的初期化 try 比前和
    ...
    # ETF 系: expense_ratio = None 初期化和、该後的代入
...
def test_yahoo_detail_no_remaining_try_only_init():
    """try block内的 `expense_ratio: Optional` 型注釈付再代入和."""
    from pathlib import Path
    src = Path(__file__).resolve().parent.parent / "src/data/yahoo_client/detail.py"
    text = src.read_text()
    # `expense_ratio: Optional[float] = _safe_get` 模式残 NG
    # (try外初期化 + try内代入 的模式変和確認)
...
# ---------------------------------------------------------------------------
# 2. save_report / save_screen / save_research / save_health 的同日上書防止
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 3. watchlist 形式不整合: list 形式和 dict 形式的両方読
# ---------------------------------------------------------------------------


def test_fallback_context_reads_list_format_watchlist(tmp_path, monkeypatch):
    """tools/watchlist.py 保存执行 list 形式 ["AAPL", "MSFT"] 読."""
