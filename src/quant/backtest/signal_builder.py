...
def _normalize_by_date(df: pd.DataFrame, raw_column: str = "raw_score") -> pd.Series:
    ...
    return df.groupby("date", sort=False)[raw_column].transform(normalize)
...
def build_single_factor_signal(
    ...
    signal = df[["date", "symbol", "universe", factor_column]].rename(columns={factor_column: "raw_score"})
    ...
    return signal[list(SIGNAL_COLUMNS)].sort_values(["date", "score", "symbol"], ascending=[True, False, True]).reset_index(
        drop=True
    )
...
def build_composite_signal(factor_values: pd.DataFrame, config: SignalConfig | None = None) -> pd.DataFrame:
    ...
    signal = pivot.reset_index()[["date", "symbol", "universe"]]
