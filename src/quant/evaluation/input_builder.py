...
def add_forward_returns(daily_bar: pd.DataFrame, periods: tuple[int, ...] | list[int] = DEFAULT_PERIODS) -> pd.DataFrame:
    ...
    prices = daily_bar[["date", "symbol", "adj_close"]].copy()
    prices["adj_close"] = pd.to_numeric(prices["adj_close"], errors="coerce")
    prices = prices.sort_values(["symbol", "date"]).reset_index(drop=True)

    grouped = prices.groupby("symbol", sort=False)["adj_close"]
...
def build_evaluation_input(
    ...
    merged = factor.merge(forward_returns, on=["date", "symbol"], how="left", validate="many_to_one")
    columns = [
        "date",
        "symbol",
        "factor_name",
    ...
    return merged[columns].sort_values(["date", "symbol"]).reset_index(drop=True)
