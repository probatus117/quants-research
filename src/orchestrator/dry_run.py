...
ROUTING_PATH = _first_existing([
    PROJECT_ROOT / ".agents/skills/stock-skills/routing.yaml",
    PROJECT_ROOT / ".claude/skills/stock-skills/routing.yaml",
])
AGENTS_DIR = _first_existing([
...
def _agents_dir_for_routing(routing_path: Path | str = ROUTING_PATH) -> Path:
...
def _agent_assets_exist(agent_name: str, agents_dir: Path | None = None) -> tuple[bool, list[str]]:
    """Return (ok, missing_paths) for the agent's md/yaml files."""
    if agent_name in ("reviewer", "history-checker"):
        # SKILL.md agent assets 
        return True, []
    base = (agents_dir or AGENTS_DIR) / agent_name
    md = base / "agent.md"
...
def _expected_tools_for_agent(agent_name: str) -> list[str]:
    ...
    tool_map = {
        ...
        "quant-researcher": ["quant_data.update", "quant_data.check",
                             "quant_factor.compute", "quant_eval.run",
...
def verify_routing(
    ...
    for ag in agents:
        ok, missing = _agent_assets_exist(ag, _agents_dir_for_routing(routing_path))
        if not ok:
            result.errors.append(
