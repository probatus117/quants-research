...
投资助手系统。集成 Yahoo Finance API（yfinance）、Grok API（xAI）和 Neo4j（GraphRAG），通过中文自然语言完成选股、公司分析、投资组合管理和风险评估。系统按 Claude Code 的 Agentic AI Pattern 运行。

## Python Environment

本项目 Python 运行环境使用 Miniconda 的 `stock-skills-2`。

Claude Code 在本仓库运行 Python / pytest / pip 时，不依赖 `conda activate`，必须使用 `conda run -n stock-skills-2 ...`。Claude Code 的命令执行可能每次启动新 shell，因此不要假设交互式 shell 已经激活环境。
...
所有 agent 输出按 4 层生成。详情见 `.claude/skills/stock-skills/SKILL.md` 的 “Output & Visibility v1” 部分。
...
| 文件 | 路径 | 何时读 |
|---|---|---|
| **Checklist（主）** | `~/Downloads/stock_skills_2_quant_implementation_checklist_selfcontained.md` | 每次编码前读取当前 Phase |
| 方案设计 | `~/Downloads/stock_skills_2_quant_extension_plan.md` | 仅设计问题/阻塞时 |
| 实装计划 | `~/Downloads/stock_skills_2_quant_implementation_plan.md` | 仅实现细节不确定时 |
...
- Codex/Claude 兼容: `.claude/` 是 Claude Code mirror，`.agents/` 是 Codex canonical，二者需要同步
