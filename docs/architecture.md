...
graph TD
    ...
    subgraph Quant["src/quant/"]
        ...
        QA["optional adapters<br/>DuckDB/Alphalens/Qlib/vectorbt<br/>Qlib native"]
        QF["factors"]
        QE["evaluation"]
        QB["backtest"]
...
4. Tools (tools/*.py)
   ...
   └─ quant_*: 因子计算、评价、TopN 回测、DuckDB/Alphalens/Qlib/vectorbt adapter、Qlib native 通路、稳健性报告、实验报告
   ↓
...
| Quant Researcher | 量化证据、因子评价、TopN 回测、成熟库 adapter、Qlib native 通路、稳健性 artifact、实验查询 | quant_data, quant_factor, quant_eval, quant_backtest, quant_qlib, quant_report, quant_experiment | Codex/Claude |
...
| quant_data.py / quant_factor.py / quant_eval.py / quant_backtest.py / quant_qlib.py / quant_report.py / quant_experiment.py | src/quant/ | 离线数据、DuckDB 查询、Qlib legacy/native conversion、因子、评价、回测、optional adapter、实验 registry 和报告 |
