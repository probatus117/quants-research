...
2. **Qlib 作为 Phase 7b 成熟研究引擎 adapter**：等 pandas MVP 有 snapshot/golden tests 后，再把标准数据转换为 Qlib 格式，复用 Qlib 的组合回测、Recorder 和实验管理能力。Qlib 不进入 Phase 0-6 / 7a P0 关键路径，但 Phase 7b 必须交付 adapter、capability check、真实/降级 artifact 和 Reviewer 检查链路。
3. **Alphalens-reloaded 作为 Phase 7b 标准单因子评价出口**：MVP 先实现最小 IC/Rank IC/分组收益导出；Alphalens 用于增强 tear sheet、换手率和 grouped analysis。依赖不可用时允许 graceful skip，但必须写入 `skip_reason`，不能把 adapter 集成视为可不做。
...
6. **vectorbt 作为 Phase 7b 快速参数实验工具**：用于 ETF 动量、少量资产和技术指标网格，不替代主流程；但在 7b 中同样按两层验收交付 adapter、grid artifact 和降级记录。
...
| 增强研究框架 | pyqlib(Qlib) | Qlib 数据格式、组合回测、实验记录 | Phase 7b 必须集成；不作为 Phase 0-6 / 7a P0 阻塞项，按 capability check + `skip_reason` 降级。 |
| 单因子评价增强 | Alphalens-reloaded | 换手、tear sheet、grouped analysis | Phase 7b 必须集成；MVP 先保留项目自有最小评价器，依赖不可用时输出 fallback artifact。 |
| 快速实验补充 | vectorbt | 参数网格、技术信号、轻量组合测试 | Phase 7b 必须集成，适合 ETF 动量和少量资产参数实验；不替代横截面因子主流程。 |
...
### Phase 7a：多市场核心链路，预计 3～4 周

目标：从单市场 fixture 升级为美/A/日三市场真实数据驱动的量化研究闭环。市场抽象必须作为 Phase 7 的第一个任务完成——它决定了 schema、provider、因子参数、回测配置和报告模板的设计方式。

任务：

```text
Phase 7.0  市场抽象 + artifact contract V0
            - schema 层引入 market 字段（daily_bar/daily_basic/calendar/dim_security/universe_member）
            - market/base_currency/benchmark 贯穿 factor_value、signal、portfolio_value、positions、trades、metrics、experiment registry、compare report
            - Provider 接口增加 market 参数；新增 get_index_member()/get_benchmark_return()
            - 新增 MarketConfig 数据类（交易日历规则、symbol 格式、货币、基准指数、默认成本参数）
            - 前置 artifact contract V0（market/data_version/base_currency/benchmark）
            - normalize_symbol() 扩展支持美股 ticker 和日股代码
            - 日股数据覆盖度评估（用 provider_probe/coverage_report 实测 yfinance 可研究范围）
            - 多市场 scale test（100/500/2000 股三级性能预估）

Phase 7.1  三市场 fixture 各自独立 + 下游 market 传播
            - tests/fixtures/quant/cn/ /us/ /jp/ 各含独立 sample 数据
            - Fixture provider 支持 market 参数
            - 三市场 fixture 跑通全流程，验证 schema → factor → eval → backtest

Phase 7.2  yfinance provider（美股 + 日股）→ artifact contract V1
            - P0 覆盖 daily_bar/calendar/benchmark return/symbol 标准化/fallback 与 skip_reason
            - daily_basic/index_member 若字段不足则 best-effort + skip_reason，不阻塞 7.2 P0
            - 字段映射（yfinance 列名 → 标准 schema）
            - artifact contract V1（provider_chain/fallback_status/skip_reason）

Phase 7.3  因子参数按市场配置化
            - config/quant_factors.yaml 中为每个因子定义各市场参数
            - 让 YAML 配置真正驱动因子计算（当前因子硬编码在 .py 中，config 形同虚设）
            - 实现 neutralize()（行业/市值中性化），先单市场后跨市场
            - zscore_neutral 纳入 factor store 持久化

Phase 7.4  回测多市场增强
            - 解除月频硬编码（weekly/monthly/quarterly）
            - benchmark 可配置（沪深300/S&P500/日经225/等权）
            - 成本模型按市场参数化
            - 回测策略抽象接口（BaseStrategy，TopN 等权/行业中性/TopK-Dropout）
            - 货币标识（base_currency）
            - Walk-forward 计算逻辑 + IC decay 计算逻辑（数据已就绪，提前到 7a）
            - 因子相关性矩阵数值计算（提前到 7a）

Phase 7.5  AKShare/Tushare provider（A 股）
            - 在 yfinance 跑通多市场链路后接入，复用已验证的 schema 和接口

Phase 7.6  Agent/Reviewer/报告层升级
            - Quant Researcher 增加三市场 agent mode（真实数据研究/fixture研究/降级研究）
            - 登记 quant_data.update/check 为正式 Agent 工具
            - 报告模板读取真实 robustness artifact（非固定文案）
            - Reviewer Phase 7 检查项（provider status/PIT/benchmark 适当性/跨市场可比性）
            - 同步 Codex/Claude 文档

Phase 7.7  测试补齐（贯穿）
            - provider mock/fallback、多市场 schema 校验、weekly/quarterly rebalance
            - zscore_neutral 持久化、robustness report 渲染、optional dependency skip
            - mocked E2E（三市场数据研究、provider fallback、跨市场对比、Reviewer Phase 7）
```

### Phase 7b：成熟量化库集成 + 研究增强，预计 2～4 周（必须交付，按两层验收）

> **定位**：7b 不是外围可选增强，而是将自研 MVP 接入行业标准技术栈，提升研究结论的可信度、可对比性和策略丰富度。
> **验收口径**（详见 `docs/plans/phase7_gap_analysis.md` G.0 节）：
> - **硬交付层（任何环境都必须通过）**：每个库都要有 adapter 代码、CLI/config 开关、capability check、artifact contract、report 集成、Reviewer 检查项、mocked/unit 测试、明确的 `skip_reason`。依赖缺失时 graceful skip 但必须留下可审计记录。
...
```text
Phase 7.8  DuckDB 集成
            - 零拷贝 Parquet 查询、增量更新、SQL 驱动的 universe 构建
            - 不可用时 fallback 到 pd.read_parquet()，写入 skip_reason

Phase 7.9  Alphalens-reloaded 集成
            - 标准 tear sheet（IC decay/quantile heatmap/turnover/autocorrelation）
            - 与 minimal_runner 交叉验证（IC 偏差 < 0.01，Phase 3 golden 已校准）
            - 不可用时 fallback 到 minimal_runner Markdown 报告，并写入 skip_reason / fallback artifact

Phase 7.10 Qlib (pyqlib) 集成
            - Qlib 格式转换（qlib_converter.py）+ 策略回测（qlib_runner.py）
            - 同策略同参数时对比 Sharpe/MaxDD/annual_return；不同策略时输出差异解释
            - PIT 数据库为未来函数问题提供基础设施
            - 不可用时 pandas MVP 独立工作，并写入 skip_reason / fallback artifact

Phase 7.11 vectorbt 集成
            - 参数网格 + ETF/技术信号快速实验
            - 不替代 A 股/美股/日股横截面因子主流程
            - 不可用时 graceful skip，并写入 skip_reason / fallback artifact

Phase 7.12 分析报告：Walk-forward + IC decay + 因子相关性（可视化与跨市场对比）

Phase 7.13 稳健性全套：市场状态分解 + 分年份/分市值/成本敏感性/TopN 敏感性
```
...
如果只用 2～3 周做出可演示版本（已完成，即 Phase 0-6），范围如下：

```text
市场：A股 synthetic sample universe（fixture 合成数据）
...
第二层：量化研究工作台
  - 多市场数据下载（美/A/日 yfinance/AKShare/Tushare + fallback）
  - 因子计算（跨市场参数自适应）
  - 单因子评价（IC/ICIR/分组收益/IC decay）
  - 多因子合成（相关性矩阵 + 权重优化）
  - pandas MVP 回测（多频率 + 可配置 benchmark + 成本按市场）
  - Walk-forward / 稳健性分析
  - Qlib / Alphalens / DuckDB / vectorbt adapters（Phase 7b 必须交付，按两层验收）
  - 实验管理
...
```text
用户：
  “用A股全市场测试 Value + Quality + Momentum 多因子，2015到现在，月频Top100，
   行业中性和非行业中性各跑一次，生成报告。”
  “在美股S&P500和日经225上用同样的因子跑一次回测，对比三市场的表现差异。”

系统：
  1. 解析任务（市场、因子、股票池、调仓频率、benchmark）
  2. 检查各市场数据是否最新（yfinance for US/JP, AKShare for CN）
  3. 按各市场参数计算因子（momentum 窗口：CN 252日/US 252日/JP 245日）
  4. 做单因子最小评价 + IC decay；如可用再生成 Alphalens tear sheet
  5. 做多因子 pandas 回测（各市场 benchmark：沪深300/S&P500/日经225）
  6. 行业中性 vs 非行业中性对比（各市场行业分类体系不同）
  7. Walk-forward 分析（expanding window）
  8. 生成 Markdown/HTML 报告（含跨市场对比表）
  9. Reviewer 检查数据泄漏、幸存者偏差、PIT 风险、benchmark 适当性
  10. 保存实验到 JSON 和 Neo4j
```
