"""DeepThink monthly cost summary (KIK-732).
...
def format_summary(month: str, summary: dict, budget_usd: float) -> str:
    lines = [f"DeepThink {month} 月度成本摘要"]
    ...
    if not by_tool:
        ...
        for tool, data in sorted(by_tool.items()):
            ...
            lines.append(
                f"  {tool}: {data['count']} / est ${data['cost_usd']:.2f}{actual_part}{err_part}"
            )
    lines.append("")
    ...
    lines.append(
        f"  合计: estimate ${total:.2f} / actual ${actual_total:.2f} / 月预算 ${budget_usd:.2f} ({pct:.0f}%)"
    )
    if pct >= 80:
        lines.append("  警告: 月预算 80% 到达")
    # KIK-737: Estimate vs Actual divergence warning (>20%)
    if actual_total > 0 and total > 0:
        divergence = abs(actual_total - total) / total * 100
        if divergence > 20:
            direction = "估算过大" if total > actual_total else "估算过小"
            lines.append(
                f"  需要再校正: {direction} {divergence:.0f}%"
            )
    return "\n".join(lines)
