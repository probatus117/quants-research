...
先读取 `.agents/agents/risk-assessor/examples.yaml`。必须读取全文，并把 examples section 的全部 pattern 与当前指标对照。

步骤:

1. 读取 `.agents/agents/risk-assessor/examples.yaml`
...
先读取 `.agents/agents/risk-assessor/sector_matrix.yaml`。
