...
解释用户的中文自然语言输入，并路由到合适的 agent。该 skill 是 Codex canonical 资产；Claude Code mirror 在 `.claude/`，后续 phase 统一同步。
...
2. 单一 `agent` -> 读取 `.agents/agents/<agent>/agent.md` 与 `examples.yaml`，按该 role 执行
3. 多个 `agents` -> 按数组顺序链式执行，并整合结果
4. 没有匹配 pattern -> 用 `agents` section 的 `role` 和 `triggers` 自主判断

## Codex Runtime Compatibility

该目录下的 SKILL.md 是 Codex canonical。`.claude/` 是 Claude Code mirror。

- Canonical Codex paths:
  - Skill: `.agents/skills/stock-skills/SKILL.md`
  - Routing: `.agents/skills/stock-skills/routing.yaml`
  - Agent roles: `.agents/agents/<agent>/agent.md`
  - Agent examples: `.agents/agents/<agent>/examples.yaml`
  - Rules: `.agents/rules/`
- `routing.yaml` 的 `agent` / `agents` 是 role file 名，不是 Codex tool 名
- 常规自然语言输入由 Codex 自身读取必需要 role file，调用 `tools/`，并按 Output & Visibility v1 回答
- 只有用户明确要求 sub-agent、并行 agent、委派等时，才使用 Codex sub-agent 功能
- `.agents` 和 `.claude` 同时存在时，Codex 优先 `.agents`
...
Codex 把 `.agents/agents/<agent>/agent.md` 和 `examples.yaml` 作为 role 定义。通常由 Codex 自身读取并执行。
...
Codex 常规执行中按独立 role task 顺序处理并 merge。只有用户明确要求 sub-agent / 并行委派时才实际同时发 sub-agent。
...
| LLM | 角色 | 范围 | 调用 |
|:---|:---|:---|:---|
| Codex | Portfolio Aligner | PF 一致性、整合判断、用户历史 | orchestrator 直接执行 |
...
API key 检测在 orchestrator 层进行，环境变量包括 `OPENAI_API_KEY`、`GEMINI_API_KEY`、`XAI_API_KEY`。只有 Codex 可用时要明说“仅用内部知识执行”。
